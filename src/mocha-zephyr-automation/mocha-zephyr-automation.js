...
const mocha_api_tests_1 = __importDefault(require("mocha-api-tests"));
const api = __importStar(require("./lib/zephyr-apis"));
const zephyrConfig = require('config');
const moment = require('moment');
class zephyrAutomation {
    config;
    zapi;
    url;
    constructor() {
        if (zephyrConfig.has('zephyrOptions.token') === false)
            console.error(`zephyrOptions.token not found in config/default.json file.`);
        if (zephyrConfig.has('zephyrOptions.environment') === false)
            console.error(`zephyrOptions.environment not found in config/default.json file.`);
        if (zephyrConfig.has('zephyrOptions.project') === false)
            console.error(`zephyrOptions.project not found in config/default.json file.`);
        if (zephyrConfig.has('zephyrOptions.cycleVersion') === false)
            console.error(`zephyrOptions.cycleVersion not found in config/default.json file.`);
        if (zephyrConfig.has('zephyrOptions.cycleFolder') === false)
            console.error(`zephyrOptions.cycleFolder not found in config/default.json file.`);
        if (zephyrConfig.has('zephyrOptions.cycleFolderHieracrhy') === false)
            console.error(`zephyrOptions.cycleFolderHieracrhy not found in config/default.json file.`);
        if (zephyrConfig.has('zephyrOptions.cycleName') === false)
            console.error(`zephyrOptions.cycleName not found in config/default.json file.`);
        if (zephyrConfig.has('zephyrOptions.updateZephyr') === false)
            console.error(`zephyrOptions.updateZephyr not found in config/default.json file.`);
        this.url = {
        ...
        this.zapi = new mocha_api_tests_1.default(this.url);
        this.config = {
     ...
     */
    async checkFoldersExist(fldrType, projKey, folderName, folderParent) {
        try {
            const folders = await api.getFolders(this.zapi, fldrType, projKey, false);
            ...
            if (err instanceof Error)
                console.error(`Could not check to see if folders exist using parentage ${folderParent} ${err.message}`);
        }
    }
    /**
     ...
     */
    async generateStepResults(test) {
        const results = [];
     ...
     */
    async generatExecutionDetails(test, error) {
    ...
    async setup(testKey) {
        ...
        if (this.config.updateZephyr.toLowerCase() === 'yes') {
            ...
            const testCase = await api.getTestCase(this.zapi, testKey, false); // 
            ...
            let cycleFolderParentId = null;
            let cycleFolderId = null;
            do {
     ...
     */
    async updateTestExecution(results) {
        // check if zephyr is to be updated
        if (this.config.updateZephyr.toLowerCase() === 'yes') {
...
exports.default = zephyrAutomation;
